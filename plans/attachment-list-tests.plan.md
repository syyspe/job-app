...
3. Write the four tests listed below.
4. In `e2e/applications.spec.ts`, between the `resume.txt` link assertion
   ...
   .not.toBeVisible()`). The delete step then runs against a row with no
   attachments, which is the same path it exercises today.
...
- **add, edit, attach a file, and delete an application** — gains a "Remove
  file" click and a not-visible assertion on the link, between the attach and
  the delete. This is the end-to-end counterpart to the unit test above: it
  proves the click reaches the API and the row re-renders without the
  attachment, which a `vi.fn()` callback can't show.
